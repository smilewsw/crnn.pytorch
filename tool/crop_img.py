...
def process_one(img_path, result_path, out_dir):
    ...
    with open(result_path) as fin:
        for i, line in enumerate(fin):
            ...
            h = max(np.linalg.norm(pts_src[1] - pts_src[0]), np.linalg.norm(pts_src[3] - pts_src[2]))
            w = max(np.linalg.norm(pts_src[3] - pts_src[0]), np.linalg.norm(pts_src[2] - pts_src[1]))
...
def main(data_base_dir, tp, result_name, out_dir):
    ...
    # p.apply_async(norm_xy, args=(base_dir, sub_dir))
    ...
    with open(osp.join(data_base_dir, tp, 'img_list.txt')) as fin:
        for name in fin:
            ...
            # p.apply_async(process_one, args=(img_path, result_name, out_dir))
            process_one(img_path, result_name, out_dir)
            break
    # p.close()
    # p.join()
...
if __name__ == '__main__':
    ...
    main(args.gt_file, args.dis_file, args.img_dir, args.dis_dir)